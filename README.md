...
Ships as `Muesli-Setup-1.0.1.exe` — a per-user installer, no admin rights needed.
...
Download `Muesli-Setup-1.0.1.exe` from the
...
Produces `dist\Muesli\Muesli.exe` and `dist\Muesli-Setup-1.0.1.exe`.
