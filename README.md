...
Ships as `Muesli-Setup-1.1.0.exe` — a per-user installer, no admin rights needed.
...
The key is configurable, on its own or with modifiers — `Win+Space`,
`Ctrl+Alt+D`, `F13`, whatever is free. **The Fn key cannot be bound:** laptop
keyboards resolve it in their own firmware, so Windows never sees it and no
application can capture it.
...
Download `Muesli-Setup-1.1.0.exe` from the
...
Produces `dist\Muesli\Muesli.exe` and `dist\Muesli-Setup-1.1.0.exe`.
